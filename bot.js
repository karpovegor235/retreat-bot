...
bot.onText(/\/start/, (msg) => {
    ...
    // Извлекаем параметр: после ?start= или после пробела
    const matchQuery = text.match(/[?&]start=([^&\s]+)/);
    if (matchQuery) {
        startParam = matchQuery[1];
    } else {
        const parts = text.split(/\s+/);
        if (parts[1]) startParam = parts[1];
    }
    
    console.log(`🔍 /start от ${chatId}, текст: "${text}", параметр: "${startParam}"`);

    // Принудительно сбрасываем старый диалог
...
// ===== ПРИГЛАСИТЬ ПОДРУГУ (отдельный обработчик, как в рабочей версии) =====
...
const inviteHandler = async (msg) => {
    ...
    // СООБЩЕНИЕ 1 – просьба передать ссылку
    await bot.sendMessage(chatId,
...
app.listen(PORT, async () => {
    ...
    const result = await fetch(`https://api.telegram.org/bot${TOKEN}/setWebhook?url=${WEBHOOK_URL}`);
    const data = await result.json();
    
    if (data.ok) {
        console.log(`✅ Webhook установлен: ${WEBHOOK_URL}`);
    } else {
        console.log(`❌ Ошибка webhook: ${data.description}`);
    }
