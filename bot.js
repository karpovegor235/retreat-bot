...
bot.onText(/\/start(?:\?start=(.+))?/, (msg, match) => {
    ...
    // Логируем параметр только для отладки
    ...
    // Сохраняем источник, если он есть
    if (source) {
        const user = getUser(chatId);
        user.source = source;
        saveUser(chatId, user);
        console.log(`✅ Новый пользователь! Источник: ${source}`);
...
// ===== ДИАЛОГ (объединённый: получение имени и приглашение подруги) =====
bot.on('message', async (msg) => {
    ...
    if (!d) return;
      ...
      // ----- Приглашение подруги -----
    if (d.step === 'invite_friend') {
        const friendName = msg.text.trim();
        if (!friendName || friendName.length < 1) {
            bot.sendMessage(chatId, '❌ Пожалуйста, напиши имя подруги (хотя бы один символ).');
            delete userDialogs[chatId];
            return;
        }
        
        const inviterName = msg.from.first_name || 'Подруга';
        const botUsername = (await bot.getMe()).username;
        const inviteLink = `https://t.me/${botUsername}?start=ref_${chatId}`;
        
        // Сохраняем приглашение в invites.json (ключ – имя подруги)
        let invites = {};
        ...
        // 1. Сообщение с просьбой передать ссылку (простой текст)
        await bot.sendMessage(chatId,
            `👭 Отлично! Теперь отправь ${friendName} эту ссылку:\n\n` +
            `🌸 ${inviterName} приглашает тебя на ретрит «Инь·Янь. Баланс»!`
        );
        await new Promise(r => setTimeout(r, 1000));
        
        // 2. Сама ссылка (простой текст, без Markdown)
        await bot.sendMessage(chatId, `🔗 Персональная ссылка со скидкой:\n${inviteLink}`);
        await new Promise(r => setTimeout(r, 1500));
        
        // 3. Инструкция для подруги и информация о скидке для обеих (простой текст)
        await bot.sendMessage(chatId,
            `📌 Что нужно сделать подруге:\n` +
            `1️⃣ Перейти по ссылке\n` +
            `2️⃣ Написать боту /start\n` +
            `3️⃣ Заполнить короткий опрос (имя, пара вопросов)\n` +
            `4️⃣ Отправить свой номер телефона (кнопка "Отправить номер")\n` +
            `5️⃣ Готово! Скидка активируется автоматически.\n\n` +
            `🎁 Что получаете ОБЕ:\n` +
            `После того как подруга забронирует ретрит, вы обе получите скидку 10% на следующий ретрит.\n\n` +
            `💚 Статус приглашения можно уточнить у организатора @${ORGANIZER_TG}`
        );
    ...
    // ----- Основной сценарий: получение имени и подарок -----
    if (d.step === 'name') {
    ...
    // Если неизвестный шаг – сбрасываем диалог
    delete userDialogs[chatId];
});
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'invite_friend_menu') {
        ...
        await bot.sendMessage(chatId,
            ...
            `Напиши её *имя* (или ник, как удобно) — я создам для неё персональную ссылку.\n\n` +
            `💡 *Важно:* Если она забронирует ретрит по этой ссылке, вы ОБЕ получите скидку 10% на следующий ретрит 🤍\n\n` +
            `Просто отправь мне её имя:`,
            { parse_mode: 'Markdown' }
        );
        await bot.answerCallbackQuery(query.id);
        return;
...
bot.on('contact', (msg) => {
    ...
    // Проверяем приглашение (используется username подруги)
    let invites = {};
    try { invites = JSON.parse(fs.readFileSync('invites.json')); } catch(e) {}
...
bot.onText(/\/sources/, (msg) => {
    ...
    try {
        ...
        for (const [id, user] of Object.entries(users)) {
            const source = user.source || 'прямой заход';
