...
// ===== НАСТРОЙКИ =====
const TOKEN = process.env.TOKEN || '8643652432:AAFRCqodyspBJsr2sJhkCPAHiKfAbeQtII0';
...
bot.on('message', (msg) => {
    ...
    if (d.step === 'name') {
        ...
        d.data.exp = msg.text;
        d.step = 'fears';
        bot.sendMessage(chatId, `Что тебя останавливает?`, {
            reply_markup: {
                inline_keyboard: [
                    [{ text: '🌙 Ничего', callback_data: 'fears_none' }],
                    [{ text: '✏️ Напишу', callback_data: 'fears_later' }]
                ]
            }
        });
    }
...
Просто отправь мне её username:`,
    ...
    if (data === 'exp_never' || data === 'exp_few' || data === 'exp_many') {
        ...
        if (d && d.step === 'experience') {
            d.data.exp = map[data];
            d.step = 'fears';
            await bot.editMessageText('Что тебя останавливает?', {
                chat_id: chatId, message_id: msgId,
                reply_markup: { inline_keyboard: [[{ text: '🌙 Ничего', callback_data: 'fears_none' }], [{ text: '✏️ Напишу', callback_data: 'fears_later' }]] }
            });
        }
        await bot.answerCallbackQuery(query.id);
        return;
...
const inviteHandler = async (msg) => {
    ...
    // ОТПРАВЛЯЕМ ОДНИМ СООБЩЕНИЕМ - ссылка будет кликабельной
    const message = `👭 *Персональная ссылка для @${friendUsername}*
